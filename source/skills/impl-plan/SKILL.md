...
If `$ARGUMENTS` begins with the literal marker `[orchestrated]`, strip the marker and skip this step entirely — you're being driven by the build orchestrator and must not block on user input. Proceed to Step 1.
